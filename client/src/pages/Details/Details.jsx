...
function Details() {
  ...
  const ownerProfileUpdate = ownerProfileStore(
    (state) => state.updateOwnerProfile
  );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      await axios
        .get(`http://localhost:3005/api/listing/?id=${listingId}`)
        .then((results) => {
          console.log('listing data', results.data);
          setType(results.data.type);
          setImages(results.data.images_urls);
          ...
          ownerProfileUpdate({
            toUserName: results.data.donor[0].username,
            toUserProfile: results.data.donor[0].photo_url,
          });
        })
        .catch((err) => console.log(err));
      await axios
        .get(`http://localhost:3005/api/username?username=${username}`)
        .then((user) => {
  ...
  return (
    <div className="details-page">
      <NavBar />
      <main>
        <Title order={1} className="type-of-transcation">
          {type ? type.toUpperCase() : 'SWAP'}
        </Title>
